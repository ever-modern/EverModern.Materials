...
public class DateTimeContstraintsSourceTests
{
    ...
    static readonly DateSlotConstraintsSource _source = new(
    ...
    public void GetConstraints_FullySpecifiedDate_ProducesDeterministicDigits()
    {
        ...
        var getConstraints = (int i) => _source.GetSlotConstraints(i, inputValue);
        ...
        for (int i = 0; i < inputValue.Length; i++)
        {
            ...
            {
                Assert.Single(getConstraints(i).Options);
                Assert.Equal(ch, getConstraints(i).Options[0]);
            }
            else
            {
                // numeric digit should be among allowed options (ideally single)
                Assert.Contains(ch, getConstraints(i).Options);
            }
        }
    }
    ...
    public void GetConstraints_FebruaryLeapYear_RestrictsDayTens()
    {
        ...
        var getConstraints = (int i) => _source.GetSlotConstraints(i, filling);
        
        var dayTens = getConstraints(0).Options;
