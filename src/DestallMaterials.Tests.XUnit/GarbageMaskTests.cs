...
public class GarbageMaskTests
{
    ...
    #region User Input Simulation Tests

    /// <summary>
    /// Helper class to simulate user input scenarios on Mask
    /// </summary>
    public class UserInputEmulator
    {
        ...
        )
        {
            ...
            if (startIndex >= 0 && startIndex < currentPart.Count)
            {
                int actualCount = Math.Min(count, currentPart.Count - startIndex);
                currentPart.RemoveRange(startIndex, actualCount);
                currentPart.InsertRange(startIndex, newContent);
            }
            else if (startIndex == currentPart.Count)
            {
                // Append at the end
        ...
        /// <summary>
        /// Resets the mask to original state
        /// </summary>
        public void Reset()
        {
        ...
        /// <summary>
        /// Gets the current state of all parts as a readable string
        /// </summary>
    ...
    public void UserInputEmulator_DateInput_RemoveCharacterFromMiddle()
    {
        ...
        // Assert - Should adapt year to closest valid format
        Assert.Equal(4, result.PartIndex);
        Assert.Equal(4, parts[4].Count); // Should still have 4 digits
    }

    [Fact]
    public void UserInputEmulator_DateInput_RemoveCharacterFromRight()
    {
        // Arrange - Setup date: 25-12-2024
        var constrainer = new DateInputConstrainer();
        var parts = new List<List<char>>
        ...
        // Assert - Should adapt year to valid format
        Assert.Equal(4, result.PartIndex);
        Assert.Equal(4, parts[4].Count); // Should still have 4 digits after adaptation
    }

    [Fact]
    public void UserInputEmulator_DateInput_TypeCharacterOnLeft()
    {
        // Arrange - Setup date: 5-12-2024 (day first digit empty)
        var constrainer = new DateInputConstrainer();
        var parts = new List<List<char>>
        {
        ...
        // Assert - Should accept valid day first digit
        Assert.Equal(0, result.PartIndex);
        Assert.Single(parts[0]);
        Assert.Equal('2', parts[0][0]);
    }
    ...
    [Fact]
    public void UserInputEmulator_DateInput_TypeCharacterInMiddle()
    {
        // Arrange - Setup date: 25-12-2024
        var constrainer = new DateInputConstrainer();
        var parts = new List<List<char>>
        ...
        // Assert - Should handle insertion appropriately
        Assert.Equal(4, result.PartIndex);
        // The mask should adapt to keep valid year format
    }

    [Fact]
    public void UserInputEmulator_DateInput_TypeCharacterOnRight()
    {
        // Arrange - Setup date: 2-12-2024 (day first digit has '2')
        var constrainer = new DateInputConstrainer();
        var parts = new List<List<char>>
        {
            new List<char> { '2' },
        ...
        // Assert - Should handle gracefully (either reject or propagate)
        Assert.True(result.PartIndex >= 0);
    }

    [Fact]
    public void UserInputEmulator_DateInput_SelectAndEraseRange()
    {
        // Arrange - Setup date: 25-12-2024
        var constrainer = new DateInputConstrainer();
        var parts = new List<List<char>>
        ...
        // Assert - Should adapt year to valid 4-digit format
        Assert.Equal(4, result.PartIndex);
        Assert.Equal(4, parts[4].Count); // Should still have 4 digits after adaptation
    }
    ...
    #region Phone Number Input User Simulation Tests

    [Fact]
    public void UserInputEmulator_PhoneInput_RemoveCharacterFromLeft()
    {
        // Arrange - Setup phone: +1 (555) 123-4567
        var constrainer = new PhoneNumberConstrainer();
        var parts = new List<List<char>>();
        for (int i = 0; i < 15; i++)
        {
        ...
        // Assert - Should handle removal and potentially affect constraints
        Assert.Equal(0, result.PartIndex);
        Assert.Single(parts[0]); // Should have '1' remaining
        Assert.Equal('1', parts[0][0]);
    }

    [Fact]
    public void UserInputEmulator_PhoneInput_RemoveCharacterFromMiddle()
    {
        // Arrange
        var constrainer = new PhoneNumberConstrainer();
        var parts = new List<List<char>>();
        ...
        // Assert - Should adapt to valid area code format
    ...
    #region Email Input User Simulation Tests

    [Fact]
    public void UserInputEmulator_EmailInput_RemoveCharacterFromLeft()
    {
        // Arrange - Setup email: user@gmail.com
        var constrainer = new EmailInputConstrainer();
        var parts = new List<List<char>>
        {
        ...
        // Assert - Should remove 'u' and leave 'ser'
        Assert.Equal(0, result.PartIndex);
        Assert.Equal(3, parts[0].Count);
        Assert.Equal("ser", new string(parts[0].ToArray()));
    }

    [Fact]
    public void UserInputEmulator_EmailInput_RemoveCharacterFromMiddle()
    {
        // Arrange
        var constrainer = new EmailInputConstrainer();
        var parts = new List<List<char>>
        ...
        // Assert - Should remove 'm' from domain
        Assert.Equal(2, result.PartIndex);
        Assert.Equal(4, parts[2].Count);
        Assert.Equal("gail", new string(parts[2].ToArray()));
    }

    [Fact]
    public void UserInputEmulator_EmailInput_RemoveCharacterFromRight()
    {
        // Arrange
        var constrainer = new EmailInputConstrainer();
        var parts = new List<List<char>>
        ...
        // Assert - Should remove 'm' from TLD
        Assert.Equal(4, result.PartIndex);
        Assert.Equal(2, parts[4].Count);
        Assert.Equal("co", new string(parts[4].ToArray()));
    }

    [Fact]
    public void UserInputEmulator_EmailInput_TypeCharacterOnLeft()
    {
        // Arrange - Start with empty local part
        var constrainer = new EmailInputConstrainer();
        var parts = new List<List<char>>
        {
        ...
        // Assert - Should add 'u' to local part
        Assert.Equal(0, result.PartIndex);
        Assert.Single(parts[0]);
    ...
    public void UserInputEmulator_EmailInput_SelectAndEraseRange()
    {
        ...
        // Assert - Should remove 'se' and leave 'ur'
        Assert.Equal(0, result.PartIndex);
    ...
    #region Complex Multi-Part User Scenarios

    [Fact]
    public void UserInputEmulator_DateInput_CompleteUserJourney()
    {
        // Arrange - Empty date input
        var constrainer = new DateInputConstrainer();
        var parts = new List<List<char>>
        {
            new List<char>(), // Day first
        ...
        emulator.TypeCharacter(4, '2', 2);
        emulator.TypeCharacter(4, '5', 3);

        // Assert - Should build complete valid date
        Assert.Equal('1', parts[0][0]); // Day first = '1'
        Assert.Equal('5', parts[1][0]); // Day second = '5'
        Assert.Equal('0', parts[2][0]); // Month first = '0'
        Assert.Equal('3', parts[3][0]); // Month second = '3'
