...
/// <summary>
/// Ordered read-only LINQ-to-entities query surface with asynchronous materialization.
/// </summary>
/// <typeparam name="T">The element type.</typeparam>
public interface IReadOnlyAsyncOrderedQueryable<T> : IAsyncMaterializable<T>
{
    /// <summary>
    /// Performs a subsequent ordering by the specified key.
    /// </summary>
    IReadOnlyAsyncOrderedQueryable<T> ThenBy<TKey>(Expression<Func<T, TKey>> keySelector);
    /// <summary>
    /// Performs a subsequent ordering by the specified key in descending order.
    /// </summary>
    IReadOnlyAsyncOrderedQueryable<T> ThenByDescending<TKey>(Expression<Func<T, TKey>> keySelector);
    /// <summary>
    ...
    /// </summary>
    IReadOnlyAsyncQueryable<TResult> GroupJoin<TInner, TKey, TResult>(
    ...
    /// </summary>
    IReadOnlyAsyncQueryable<TResult> LeftJoin<TInner, TKey, TResult>(
        IReadOnlyAsyncOrderedQueryable<TInner> inner,
        Expression<Func<T, TKey>> outerKeySelector,
        Expression<Func<TInner, TKey>> innerKeySelector,
